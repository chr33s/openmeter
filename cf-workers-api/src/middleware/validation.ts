...
export const commonSchemas = {
  ...
  namespace: z.string().min(1).max(64).regex(
    /^[a-z0-9]+(?:[-_][a-z0-9]+)*$/,
...
export const validateRequestId = () => createMiddleware<{ Bindings: Env; Variables: { requestId: string } }>(
  async (c, next) => {
...
export const validateIdempotencyKey = () => createMiddleware<{ 
  Bindings: Env; 
...
export const validateBodySize = (maxSize: number = 1024 * 1024) => // 1MB default
  createMiddleware<{ Bindings: Env }>(async (c, next) => {
