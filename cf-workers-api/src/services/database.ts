...
export const meters = sqliteTable(
  'meters',
  {
    ...
    groupBy: text('group_by', { mode: 'json' }).$type<Record<string, string>>().default({}),
  ...
  (table) => ({
    ...
    namespaceKeyIdx: index('idx_meters_namespace_key').on(table.namespace, table.key),
...
export const subjects = sqliteTable(
  ...
  (table) => ({
    namespaceKeyUnique: unique().on(table.namespace, table.key),
    namespaceKeyIdx: index('idx_subjects_namespace_key').on(table.namespace, table.key),
  })
);
...
export const events = sqliteTable(
  ...
  (table) => ({
    meterSubjectIdx: index('idx_events_meter_subject').on(table.meterId, table.subjectId),
    timestampIdx: index('idx_events_timestamp').on(table.timestamp),
    meterTimestampIdx: index('idx_events_meter_timestamp').on(table.meterId, table.timestamp),
    meterFk: foreignKey({
...
export const features = sqliteTable(
  ...
  (table) => ({
    namespaceKeyUnique: unique().on(table.namespace, table.key),
    namespaceKeyIdx: index('idx_features_namespace_key').on(table.namespace, table.key),
    meterFk: foreignKey({
      columns: [table.meterId],
      foreignColumns: [meters.id],
    }),
...
export const usageAggregates = sqliteTable(
  ...
  (table) => ({
    ...
    meterSubjectIdx: index('idx_usage_aggregates_meter_subject').on(table.meterId, table.subjectId),
