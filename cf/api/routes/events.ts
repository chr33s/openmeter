...
import { validate, commonSchemas, validateIdempotencyKey } from '@/middleware/validation';
...
app.get('/',
  ...
  async (c) => {
    ...
    try {
      ...
      const cacheKey = CacheService.createKey('events', namespace, JSON.stringify(query));
      ...
      return addPaginationHeaders(httpResponse, c.req.url, paginationParams, result.totalCount);
    } catch (error) {
...
async function checkIdempotency(
  ...
  const cacheKey = CacheService.createKey('idempotency', namespace, operation, key);
