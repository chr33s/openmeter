...
export class EventsService {
  ...
  async ingestEvent(
    ...
    try {
      ...
      const subject = await this.findOrCreateSubject(namespace, eventData.subject);
  ...
  async queryEvents(params: {
    ...
    const database = this.db.database;
    const { namespace, meterId, subjectId, from, to, limit = 100, offset = 0 } = params;
  ...
  // Find or create subject
  private async findOrCreateSubject(namespace: string, subjectKey: string): Promise<Subject> {
    const database = this.db.database;
    ...
    try {
      ...
      const existingSubjects = await database
        ...
        .from(subjects)
        .where(and(
          eq(subjects.namespace, namespace),
          eq(subjects.key, subjectKey)
        ))
        .limit(1);
  ...
  // Find meter by event type
  private async findMeterByEventType(namespace: string, eventType: string): Promise<Meter | null> {
    const database = this.db.database;
  ...
  private extractEventValue(eventData: IngestEventRequest, meter: Meter): number {
