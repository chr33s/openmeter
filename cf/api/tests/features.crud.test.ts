...
describe("Features CRUD", () => {
	test("POST creates feature, GET returns it; duplicate -> 409", async () => {
		...
		const getRes = await SELF.fetch(
			`http://localhost/api/v1/features/${created.id}`,
		);
	...
	test("PUT updates; DELETE soft-deletes; 404 for missing", async () => {
		...
		const create = await SELF.fetch("http://localhost/api/v1/features", {
			...
			headers,
			body: JSON.stringify({
				key: `f_${crypto.randomUUID().slice(0, 8)}`,
				name: "X",
			}),
		});
		...
		const putRes = await SELF.fetch(
			`http://localhost/api/v1/features/${feat.id}`,
			{
				method: "PUT",
				headers,
		...
		const missing = await SELF.fetch(
			`http://localhost/api/v1/features/${crypto.randomUUID()}`,
			{
				headers,
			},
		);
