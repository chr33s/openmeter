...
class ApiClient {
  ...
  // Events endpoints
  async getEvents(params: EventsQueryParams = {}): Promise<PaginationResponse<Event>> {
    const query = this.buildQuery(params)
  ...
  // Meters endpoints
  async getMeters(params: { search?: string; page?: number; pageSize?: number } = {}): Promise<PaginationResponse<Meter>> {
    const query = this.buildQuery(params)
  ...
  async queryMeter(id: string, params: MeterQueryParams): Promise<MeterQueryResult> {
    const query = this.buildQuery(params)
    return this.request<MeterQueryResult>(`/api/v1/meters/${id}/query?${query}`)
  }

  // Subjects endpoints
  async getSubjects(params: { search?: string; page?: number; pageSize?: number } = {}): Promise<PaginationResponse<Subject>> {
    const query = this.buildQuery(params)
    return this.request<PaginationResponse<Subject>>(`/api/v1/subjects?${query}`)
  }
  ...
  async getEntitlements(params: { subjectId?: string; page?: number; pageSize?: number } = {}): Promise<PaginationResponse<Entitlement>> {
    const query = this.buildQuery(params)
    // Note: This endpoint may not exist in the actual API, using as placeholder
    return this.request<PaginationResponse<Entitlement>>(`/api/v1/entitlements?${query}`)
  }

  // Plans endpoints (placeholder)
  async getPlans(params: { page?: number; pageSize?: number } = {}): Promise<PaginationResponse<Plan>> {
    const query = this.buildQuery(params)
    // Note: This endpoint may not exist in the actual API, using as placeholder
