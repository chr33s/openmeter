...
describe("Documentation", () => {
	test("GET /docs returns API documentation", async () => {
		const response = await SELF.fetch("http://localhost/docs");
		expect(response.status).toBe(200);
...
describe("Metrics", () => {
	test("GET /metrics returns metrics in JSON format", async () => {
		const response = await SELF.fetch("http://localhost/metrics");
		expect(response.status).toBe(200);
...
describe("Request Validation", () => {
	test("POST with invalid content type returns 400", async () => {
		const response = await SELF.fetch("http://localhost/api/v1/events", {
			method: "POST",
			headers: {
